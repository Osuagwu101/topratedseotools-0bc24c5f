    ...
    <SiteLayout>
      <div className="mx-auto max-w-lg px-4 py-24 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          This link may be invalid or unavailable. Please contact the person who sent it to you.
        </p>
      </div>
    </SiteLayout>
  ),
...
function CustomPaymentPage() {
  ...
  const [verifyState, setVerifyState] = useState<
    "idle" | "checking" | "paid" | "pending" | "error"
  >(link.status === "paid" ? "paid" : "idle");
    ...
    verifyPayment({
      ...
      .then(async (result) => {
        ...
        if (result.status === "pending") {
          ...
          setMessage(
            "Payment verification pending. Refresh this page in a moment — nothing is confirmed yet.",
          );
          return;
        }
  ...
  const expiryText = useMemo(
    () => (link.expires_at ? new Date(link.expires_at).toLocaleString() : null),
    [link.expires_at],
  );
  ...
  async function pay() {
    ...
    try {
      const result = await initPayment({
        data: { token, payer_name: payerName.trim(), payer_email: payerEmail.trim() },
      });
          ...
          <div className="rounded-3xl border bg-card p-6 shadow-card sm:p-8">
            ...
            <div className="mt-6 rounded-2xl border bg-background/50 p-5 text-center">
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                One-time payment · charged in {link.currency}
              </div>
            </div>
            ...
            {effectiveStatus === "paid" ? (
              <div className="mt-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Thank you. This payment has been verified and recorded.
                </p>
                {gatewayIdentifier ? (
                  <p className="mt-2 break-all font-mono text-[11px] text-muted-foreground">
                    {link.payment_gateway === "flutterwave"
              ...
              <div className="mt-6 rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm">
                {effectiveStatus === "expired"
                  ...
                  : "This payment link has been disabled. Please contact the sender."}
              </div>
            ) : (
              <div className="mt-6 space-y-4">
                {verifyState === "checking" ? (
                  <div className="flex items-center justify-center gap-2 rounded-xl border p-4 text-sm text-muted-foreground">
                    <Loader2 className="h-4 w-4 animate-spin" /> Confirming your payment with{" "}
                    {providerName}…
                  </div>
                ) : (
                  <>
                    <div>
                      <label
                        htmlFor="custom-payment-payer-name"
                        className="mb-1 block text-sm font-medium"
                      >
                        Name
                      </label>
                      <input
                        id="custom-payment-payer-name"
                        name="name"
                        value={payerName}
                        onChange={(e) => setPayerName(e.target.value)}
                        autoComplete="name"
                        className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/30"
                        placeholder="Your name"
                      />
                    </div>
                    <div>
                      <label
                        htmlFor="custom-payment-payer-email"
                        className="mb-1 block text-sm font-medium"
                      >
                        Email
                      </label>
                      <input
                        id="custom-payment-payer-email"
                        name="email"
                        type="email"
                        value={payerEmail}
                    ...
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-gradient-primary px-4 py-3 text-sm font-semibold text-primary-foreground shadow-glow hover:opacity-90 disabled:opacity-60"
                    >
                      {busy ? (
            ...
            {message && verifyState !== "paid" ? (
              <p
                className={`mt-4 text-center text-sm ${verifyState === "error" ? "text-destructive" : "text-muted-foreground"}`}
                role="status"
                aria-live="polite"
              >
                {message}
            ...
            <div className="mt-6 border-t pt-4 text-center text-xs text-muted-foreground">
              <p className="inline-flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5" /> Secure payment processed by {providerName}
              </p>
              {expiryText && effectiveStatus === "active" ? (
                <p className="mt-1">Link expires {expiryText}</p>
              ) : null}
            </div>
          </div>
