...
export const Route = createFileRoute("/admin/settings/browser-auth")({
  ssr: false,
  head: () => ({
    meta: [{ title: "One-Click Browser Login — Admin" }, { name: "robots", content: "noindex" }],
  }),
  beforeLoad: async () => {
    await requireAdminOrRedirect();
  },
  loader: async ({ context }) => {
    await context.queryClient.ensureQueryData(browserAuthQuery);
  },
...
function BrowserAuthAdminPage() {
  ...
  async function saveGeneral() {
    ...
    try {
      await updateSettings({
        data: { enabled, default_provider: provider, session_timeout_minutes: timeout },
      });
  ...
  async function saveProviderSecrets(p: BrowserAuthProvider) {
    ...
    const secrets = Object.fromEntries(
      fields.filter((f) => draft[f]?.trim()).map((f) => [f, draft[f].trim()]),
    );
      ...
      <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Configure Browser Use and Cloudflare Browser Run. API credentials are write-only and
        ...
        <div className="rounded-2xl border bg-card p-5 shadow-card">
          <div className="flex items-center gap-2 text-sm font-semibold">
            <ShieldCheck className="h-4 w-4" /> Global behaviour
          </div>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <label className="flex items-center gap-3 text-sm sm:col-span-2">
              ...
              />
              <span>
                <b>Enable One-Click Login</b> for tools whose per-tool switch is enabled.
              </span>
            </label>
            <label className="text-sm">
              ...
              <select
                value={provider}
                onChange={(e) => setProvider(e.target.value as BrowserAuthProvider)}
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Leave a tool's provider blank to use this default.
              </p>
            </label>
            <label className="text-sm">
              <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              ...
              <Input
                ...
                value={timeout}
                onChange={(e) =>
                  setTimeoutMinutes(Math.max(5, Math.min(60, Number(e.target.value) || 30)))
                }
              />
          ...
          <Button
            className="mt-4"
            onClick={saveGeneral}
            disabled={busy === "settings" || !data.is_super_admin}
          >
            {busy === "settings" ? "Saving…" : "Save settings"}
          </Button>
          {!data.is_super_admin && (
            <span className="ml-2 text-xs text-muted-foreground">Super Admin only</span>
          )}
        </div>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          <ProviderCard
            title="Browser Use"
            description="Private cloud browser with an interactive customer Live View."
            configured={info("browser_use").configured}
            missing={info("browser_use").missing_secrets}
            busy={busy}
            provider="browser_use"
            isSuper={data.is_super_admin}
            onTest={runTest}
            onSave={saveProviderSecrets}
          >
            <SecretField
              label="Browser Use API key"
              saved={info("browser_use").configured_secrets.includes("BROWSER_USE_API_KEY")}
              value={draft.BROWSER_USE_API_KEY ?? ""}
              onChange={(v) => setDraft((d) => ({ ...d, BROWSER_USE_API_KEY: v }))}
              placeholder="bu_…"
            />
          </ProviderCard>
          ...
          <ProviderCard
            ...
            description="Cloudflare remote browser with interactive tab-mode Live View."
            configured={info("cloudflare").configured}
            missing={info("cloudflare").missing_secrets}
            busy={busy}
            provider="cloudflare"
            isSuper={data.is_super_admin}
            onTest={runTest}
          ...
          >
            <SecretField
              ...
              saved={info("cloudflare").configured_secrets.includes("CLOUDFLARE_ACCOUNT_ID")}
              value={draft.CLOUDFLARE_ACCOUNT_ID ?? ""}
              onChange={(v) => setDraft((d) => ({ ...d, CLOUDFLARE_ACCOUNT_ID: v }))}
              placeholder="Account ID"
            />
            <SecretField
              label="Browser Run API token"
              saved={info("cloudflare").configured_secrets.includes(
                "CLOUDFLARE_BROWSER_RUN_API_TOKEN",
              )}
              value={draft.CLOUDFLARE_BROWSER_RUN_API_TOKEN ?? ""}
              onChange={(v) => setDraft((d) => ({ ...d, CLOUDFLARE_BROWSER_RUN_API_TOKEN: v }))}
              placeholder="Browser Rendering Edit token"
            />
          </ProviderCard>
        </div>

        <div className="mt-5 rounded-2xl border border-dashed bg-muted/20 p-4 text-xs text-muted-foreground">
          <b className="text-foreground">Customer flow:</b> the server verifies a successful active
          subscription, loads only that order's assigned account, injects its stored login
          credentials into the remote browser, submits the login form, and returns only the
          provider's signed Live View URL. CAPTCHA/2FA can be completed inside that same remote tab
          after handoff.
        </div>
      </section>
    </AdminShell>
...
function ProviderCard({
  ...
  isSuper: boolean;
  onTest: (p: BrowserAuthProvider) => void;
  onSave: (p: BrowserAuthProvider) => void;
  ...
  return (
    <div className="rounded-2xl border bg-card p-5 shadow-card">
      <div className="flex items-center justify-between gap-3">
        ...
        <span
          className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase ${configured ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}
        >
          {configured ? "Configured" : "Not ready"}
        </span>
      </div>
      <div className="mt-4 space-y-3">{children}</div>
      {missing.length > 0 && (
        <p className="mt-3 text-xs text-destructive">Missing: {missing.join(", ")}</p>
      )}
      <div className="mt-4 flex flex-wrap gap-2">
        <Button
          size="sm"
          onClick={() => onSave(provider)}
          disabled={!isSuper || busy === `save:${provider}`}
        >
          <KeyRound className="mr-1 h-3.5 w-3.5" />{" "}
          {busy === `save:${provider}` ? "Saving…" : "Save & validate"}
        </Button>
        <Button
          size="sm"
          variant="outline"
  ...
  return (
    <div>
      ...
      <Input
        ...
        placeholder={saved ? "Enter a new value to replace" : placeholder}
      />
    </div>
  );
}
