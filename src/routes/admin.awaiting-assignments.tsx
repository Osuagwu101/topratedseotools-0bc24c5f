...
export const Route = createFileRoute("/admin/awaiting-assignments")({
  ...
  head: () => ({
    meta: [{ title: "Awaiting assignment — Admin" }, { name: "robots", content: "noindex" }],
  }),
    ...
    <AdminShell>
      <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Paid customers who don't yet have a pool assignment. They see "Awaiting account
          assignment" — never legacy or unrelated credentials.
        </p>
              ...
              {data.rows.map((r) => (
                <tr key={r.order_id} className="border-b last:border-0 align-top">
                  ...
                  <td className="px-3 py-2 text-xs">
                    {r.available_accounts.length === 0 ? (
                      <span className="text-muted-foreground">None</span>
                    ) : (
                      r.available_accounts.map((a: any) => (
                        <div key={a.id}>
                          {a.label}{" "}
                          <span className="text-muted-foreground">({a.available} free)</span>
                        </div>
                      ))
                    )}
                  </td>
