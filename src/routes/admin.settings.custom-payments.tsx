...
export const Route = createFileRoute("/admin/settings/custom-payments")({
  ssr: false,
  head: () => ({
    meta: [{ title: "Custom Payments — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: Page,
});
...
function Page() {
  ...
  const query = useQuery({
    queryKey: ["custom-payment-links"],
    queryFn: () => adminListCustomPaymentLinks(),
  });
  ...
  const create = useMutation({
    ...
    onSuccess: async (r) => {
      ...
      try {
        ...
        toast.success(
          `${r.payment_gateway === "flutterwave" ? "Flutterwave" : "Paystack"} payment link created and copied`,
        );
      } catch {
  ...
  const toggle = useMutation({
    mutationFn: (d: { id: string; status: "active" | "disabled" }) =>
      adminSetCustomPaymentLinkStatus({ data: d }),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["custom-payment-links"] });
      ...
      <section className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold">Custom Payments</h1>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            Create a one-time bill, choose its payment gateway and charge currency, then share the
            generated link. Custom Payments never grant tool access.
          </p>
        </div>
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-card">
            ...
            <div className="mt-4 space-y-4">
              <F label="Payment gateway">
                <div className="grid grid-cols-2 gap-2">
                  <GatewayButton
                    active={gateway === "paystack"}
                    title="Paystack"
                    note="NGN only"
                    onClick={() => changeGateway("paystack")}
                  />
                  <GatewayButton
                    active={gateway === "flutterwave"}
                    title="Flutterwave"
                    note="Multi-currency"
                    onClick={() => changeGateway("flutterwave")}
                  />
                </div>
              </F>
                ...
                {gateway === "paystack" ? (
                  <div className="rounded-lg border bg-muted/20 px-3 py-2.5 text-sm">
                    ...
                    <span className="text-xs text-muted-foreground">
                      · Nigerian Naira (fixed for Paystack)
                    </span>
                  </div>
                ) : (
                  <div className="space-y-3 rounded-xl border bg-muted/15 p-3">
                    <div className="relative">
                      ...
                      <input
                        ...
                        aria-label="Search Flutterwave currencies"
                      />
                    </div>
                    {currencySearch.trim() ? (
                      <div>
                        ...
                        {searchedCurrencies.length ? (
                          ...
                          <div className="rounded-lg border border-dashed p-3 text-center text-xs text-muted-foreground">
                            No supported Flutterwave currency matches “{currencySearch}”.
                          </div>
                        )}
                      </div>
                    ) : (
              ...
              <div className="rounded-xl border bg-muted/25 p-3">
                <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                  {gateway === "flutterwave" ? "Flutterwave" : "Paystack"} charge
                </div>
                <div className="mt-1 text-xl font-bold">
                  {numericAmount > 0
                    ? formatCustomPaymentMoney(numericAmount, currency)
                    : "Enter an amount"}
                </div>
                {selectedCurrency ? (
                  <div className="mt-1 text-xs text-muted-foreground">
                    {selectedCurrency.name} · payer is charged in {selectedCurrency.code}
                  </div>
                ) : null}
                <div className="mt-3 rounded-lg border bg-background/70 p-2.5 text-xs text-muted-foreground">
                  No currency estimator or automatic FX conversion is applied here. The checkout is
                  created in the exact currency and amount selected above.
                </div>
              </div>
              ...
              {currenciesQuery.isLoading ? (
                <div className="flex items-center gap-2 rounded-lg border p-3 text-xs text-muted-foreground">
                  ...
                  Loading {gateway === "flutterwave" ? "Flutterwave" : "Paystack"} currencies…
                </div>
              ) : null}
              {currenciesQuery.isError ? (
                <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-xs text-destructive">
                  Could not load {gateway === "flutterwave" ? "Flutterwave" : "Paystack"} for Custom
                  Payments.{" "}
              ...
              <F label="Recipient name (optional)">
                <input
                  className={inputCls}
                  value={recipientName}
              ...
              <F label="Recipient email (optional)">
                <input
                  className={inputCls}
                  type="email"
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-gradient-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-50"
              >
                {create.isPending ? (
            ...
            {latestUrl && (
              <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-3">
                <div className="text-xs font-semibold text-primary">READY TO SEND</div>
                <div className="mt-1 break-all text-xs">{latestUrl}</div>
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-card">
            ...
            {query.isLoading ? (
              ...
              <div className="mt-4 space-y-3">
                {links.map((l: any) => {
                  const last = txs.find((t: any) => t.link_id === l.id);
                  return (
                    <div key={l.id} className="rounded-xl border p-4">
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide">
                              {l.payment_gateway === "flutterwave" ? "Flutterwave" : "Paystack"}
                            </span>
                          </div>
                          <div className="mt-1 text-sm font-bold">
                            {formatCustomPaymentMoney(Number(l.amount), String(l.currency))}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {l.recipient_name || "Any recipient"}
                            {l.recipient_email ? ` · ${l.recipient_email}` : ""}
                          </div>
                        </div>
                        <span className="h-fit rounded-full bg-muted px-2 py-0.5 text-[11px] font-semibold">
                          {l.public_status}
                        </span>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button
                          onClick={() => copy(l.payment_url)}
                          className="rounded border px-2 py-1 text-xs"
                        >
                          <Copy className="mr-1 inline h-3 w-3" />
                          Copy
                        ...
                        {l.public_status !== "paid" && (
                          <button
                            onClick={() =>
                              toggle.mutate({
                                id: l.id,
                                status: l.status === "disabled" ? "active" : "disabled",
                              })
                            }
                            className="rounded border px-2 py-1 text-xs"
                          >
                      ...
                      {last && (
                        <div className="mt-3 border-t pt-2 text-[11px] text-muted-foreground">
                          Latest: {last.status} ·{" "}
                          {last.payment_gateway === "flutterwave" ? "Flutterwave" : "Paystack"} ·{" "}
                          {last.payer_email} ·{" "}
                          {formatCustomPaymentMoney(Number(last.amount), String(last.currency))}{" "}
                          <div className="mt-1 font-mono">
                            Gateway ref: {last.gateway_reference || "—"} · Gateway txn ID:{" "}
                            {last.gateway_transaction_id || "—"} · env {last.gateway_environment}
                          </div>
                          <div className="font-mono opacity-70">
                            INTERNAL correlation key: {last.merchant_reference || last.reference}
                          </div>
                        </div>
                      )}
                      {l.status === "paid" && (
                        <div className="mt-1 text-[11px] text-emerald-700">
                          Paid · gateway ref{" "}
                          <span className="font-mono">{l.paid_gateway_reference || "—"}</span> ·
                          gateway txn ID{" "}
                          <span className="font-mono">{l.paid_gateway_transaction_id || "—"}</span>
                        </div>
                      )}
                    </div>
                  );
...
function GatewayButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-xl border p-3 text-left transition ${active ? "border-primary bg-primary/10 ring-1 ring-primary/30" : "bg-background hover:border-primary/40"}`}
    >
...
function CurrencyButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-lg border px-3 py-2 text-left transition ${active ? "border-primary bg-primary/10 ring-1 ring-primary/20" : "bg-background hover:border-primary/40"}`}
    >
