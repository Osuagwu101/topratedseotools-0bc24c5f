    ...
    <div className="grid gap-6 lg:grid-cols-[460px_1fr]">
      ...
      <div className="rounded-2xl border bg-card p-5 shadow-card">
        ...
        <div className="mt-4 space-y-3">{links.map((l:any)=>{const last=txs.find((t:any)=>t.link_id===l.id);return <div key={l.id} className="rounded-xl border p-4">
          ...
          {last&&<div className="mt-3 border-t pt-2 text-[11px] text-muted-foreground">Latest: {last.status} · {last.payment_gateway === "flutterwave" ? "Flutterwave" : "Paystack"} · {last.payer_email} · {formatCustomPaymentMoney(Number(last.amount), String(last.currency))} <div className="mt-1 font-mono">Gateway ref: {last.gateway_reference || "—"} · Gateway txn ID: {last.gateway_transaction_id || "—"} · env {last.gateway_environment}</div><div className="font-mono opacity-70">INTERNAL correlation key: {last.merchant_reference || last.reference}</div></div>}
          {l.status==="paid"&&<div className="mt-1 text-[11px] text-emerald-700">Paid · gateway ref <span className="font-mono">{l.paid_gateway_reference||"—"}</span> · gateway txn ID <span className="font-mono">{l.paid_gateway_transaction_id||"—"}</span></div>}
        </div>})}</div>}
      </div>
    </div>
  </section></AdminShell>;
