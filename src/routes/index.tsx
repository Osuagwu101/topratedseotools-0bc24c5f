...
const FAQ = [
  {
    ...
    a: "Each tool is subscribed to separately. Access types and billing periods vary by tool, and you can manage all active subscriptions from the same dashboard.",
  },
  {
  ...
  },
  {
    ...
    a: "Paystack is the default active payment gateway. Another configured gateway is used only if a Super Admin deliberately makes it active.",
  },
  {
    q: "How do renewals work?",
    a: "Eligible recurring plans renew automatically until renewal is disabled. You can disable renewal and keep access until the end of the paid billing period. Pay-per-use tools such as Turnitin do not renew.",
  },
  {
    q: "How is Shared Access activated?",
    a: "A verified successful payment activates the order immediately. If login credentials are not active yet, your subscriptions page shows the Admin WhatsApp contact so access can be completed.",
  },
  {
    q: "How is Private Access activated?",
    a: "A verified successful payment activates the order, while dedicated login fulfilment may remain pending. Contact Admin through WhatsApp if credentials have not yet been assigned.",
  },
  {
    q: "How does Turnitin work?",
    a: "Turnitin Checks are priced per check (₦2,300 each), paid once. There is no recurring billing and no automatic renewal. Message us with the number of checks you need and we'll send a payment link.",
  },
  {
  ...
  const showcase = catalog.slice(0, 8).map((t) => {
    if (t.pricingModel === "per_use" && t.perUse)
      return {
        tool: t,
        baseLines: [],
        perUseLabel: `${money.fmt(t.perUse.amount)} / ${t.perUse.unit}`,
      };
        ...
        <div className="mx-auto max-w-7xl px-4 pb-20 pt-16 sm:px-6 sm:pt-24 lg:px-8 lg:pt-32">
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border bg-background/60 px-3 py-1 text-xs font-medium text-muted-foreground backdrop-blur">
              <Sparkles className="h-3.5 w-3.5 text-primary" /> Premium software subscriptions,
              priced individually
            </div>
            <h1 className="text-4xl font-bold tracking-tight sm:text-6xl">
              Subscribe to the <span className="text-gradient-primary">premium tools you need</span>
              —without paying for a bundle.
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
              Choose from leading SEO, AI, writing, research, design, and productivity tools. Access
              types and billing periods vary by product, with clear pricing and fast activation
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Clear per-tool pricing · Paystack is the default gateway · Successful payments
              activate immediately · WhatsApp and email support
            </p>
          </div>
        </div>
      </section>
          ...
          {[
            {
              icon: Zap,
              title: "Pay per tool",
              body: "No forced software bundle. Choose only the tools you need and subscribe to each one separately.",
            },
            {
              icon: Shield,
              title: "One dashboard",
              body: "Manage every subscription, renewal, and access detail from one place — even when you subscribe to several tools.",
            },
            {
              icon: Users,
              title: "WhatsApp & email support",
              body: "Talk to a real person for activation, renewals, and account questions during business hours.",
            },
          ].map((v) => (
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-3xl font-bold tracking-tight">
            Premium tools without the forced bundle
          </h2>
          <p className="mt-3 text-muted-foreground">
            Subscribe separately to the tools that support your work. Add or remove subscriptions as
            your needs change, while managing everything from one account.
          </p>
        </div>
                ...
                {perUseLabel ? (
                  <>
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      One-time · pay per {tool.perUse!.unit}
                    </div>
                  </>
                ) : baseLines.length > 0 ? (
                  <div className="space-y-1">
                    {baseLines.map((line) => (
                      ...
                      >{`${line.title} from ${money.fmt(line.amount)}/month`}</div>
                    ))}
                  </div>
                ) : (
      ...
      <section className="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        <div className="overflow-hidden rounded-3xl border bg-gradient-primary p-10 text-center text-primary-foreground shadow-glow sm:p-16">
          ...
          <p className="mx-auto mt-3 max-w-xl text-primary-foreground/80">
            Create a free account, subscribe only to the tools you need, and manage every
            subscription from one dashboard.
          </p>
