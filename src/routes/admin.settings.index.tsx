...
export const Route = createFileRoute("/admin/settings/")({
  ssr: false,
  head: () => ({ meta: [{ title: "Settings Overview — Admin" }, { name: "robots", content: "noindex" }] }),
  loader: async ({ context }) => { await context.queryClient.ensureQueryData(overviewQuery); },
        ...
        <div className="grid grid-cols-[minmax(0,1fr)] gap-4 md:grid-cols-2">
              ...
              <ul className="divide-y">
                {data.attention.map((item) => (
                  <li key={item.id} className="flex items-center justify-between gap-3 py-2 text-sm">
                    ...
                    <span className="shrink-0 rounded-full bg-destructive/10 px-2 py-0.5 text-xs font-semibold text-destructive">{item.count}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <Card icon={<Activity className="h-4 w-4" />} title="Recent Admin Activity">
            {data.recentActivity.length === 0 ? (
              <p className="text-sm text-muted-foreground">No recent activity visible to your role.</p>
            ) : (
              <ul className="divide-y text-sm">
                {data.recentActivity.map((r) => (
                  <li key={r.id} className="flex items-center justify-between gap-3 py-2">
                    ...
                    <span className={`shrink-0 text-xs ${r.success ? "text-emerald-600" : "text-destructive"}`}>{r.success ? "ok" : "failed"}</span>
                  </li>
                ))}
              </ul>
            )}
            ...
            <ul className="divide-y text-sm">
              {data.phaseProgress.map((p) => (
                <li key={p.phase} className="flex items-center justify-between gap-3 py-2">
                  ...
                  <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold ${p.status === "Active" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>{p.status}</span>
                </li>
              ))}
            </ul>
          </Card>
...
function Card({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return (
    <div className="min-w-0 rounded-2xl border bg-card p-5 shadow-card">
      <div className="mb-3 inline-flex items-center gap-2 text-sm font-semibold">{icon} {title}</div>
      {children}
    </div>
  );
}
