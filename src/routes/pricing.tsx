  ...
  head: () => ({
    meta: [
      { title: "Pricing — Top Rated SEO Tools" },
      {
        name: "description",
        content:
          "Compare the Shared, Private and billing-period options currently available for each premium tool. Each tool renews separately — no forced bundle.",
      },
      { property: "og:title", content: "Individual Plans for Every Tool" },
      {
        property: "og:description",
        content:
          "Choose from the access types and billing periods available for each tool, or contact Admin where pricing is custom.",
      },
    ],
  }),
...
function PricingPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return grouped.filter((g) => {
      ...
      return (
        !!t && (t.name.toLowerCase().includes(needle) || t.category.toLowerCase().includes(needle))
      );
    });
  }, [grouped, q]);
  ...
  return (
    <SiteLayout>
      <section className="bg-gradient-hero">
        <div className="mx-auto max-w-4xl px-4 py-20 text-center sm:px-6 lg:px-8">
          ...
          <p className="mt-4 text-lg text-muted-foreground">
            Access type and billing periods vary by tool. Choose from the options currently shown
            for that tool, or contact Admin where pricing is custom. Every subscription is managed
            separately — no forced software bundle.
          </p>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-20 sm:px-6 lg:px-8">
        ...
        {filtered.length === 0 && (
          <p className="mt-10 text-center text-sm text-muted-foreground">No tools match "{q}".</p>
        )}
          ...
          <dl className="mt-6 space-y-4">
            {[
              ...
              },
              {
                ...
                a: "Choose from the access and billing options shown for that tool, then pay securely through the active payment gateway. Successful payments activate the order immediately. If login credentials are not active yet, your subscription page shows the Admin WhatsApp contact so access can be completed.",
              },
              {
              ...
              },
              {
                ...
                a: "Those tools currently use Admin-confirmed pricing or fulfilment. Message us and we'll confirm the available option.",
              },
              {
                q: "Can I change billing period later?",
                a: "Where a tool offers more than one billing period, you can choose another available period when renewing.",
              },
              {
    ...
    <div className="flex flex-col rounded-2xl border bg-card p-6 shadow-card transition hover:-translate-y-0.5 hover:border-primary/40">
      ...
      <div className="mt-5 flex-1 rounded-lg border bg-background/40 p-3">
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">
          One-time payment. No recurring billing, Shared/Private selection or automatic renewal.
        </p>
      </div>
      <div className="mt-5 flex gap-2">
        <Link
