  ...
  .inputValidator((input) =>
    z
      .object({
        ...
        viewport_height: z.number().int().min(320).max(1200).optional(),
      })
      .parse(input),
  )
  .handler(async ({ data, context }) => {
    ...
    if (data.grant_access) {
      ...
      const { data: orders } = await (admin as any)
        .from("tool_orders")
        .select("id, expires_at, access_type, fulfilment_status")
        .eq("user_id", context.userId)
        .eq("tool_slug", data.tool_slug)
        .eq("status", "approved")
      ...
      const order = (orders ?? []).find(
        (o: any) =>
          unexpired(o.expires_at) &&
          (o.access_type !== "private" || o.fulfilment_status === "active"),
      );
      if (!order)
        throw new Error("A successful active subscription is required for One-Click Login.");
      orderId = order.id;
    ...
    const { data: account } = await (admin as any)
      ...
      .eq("tool_slug", data.tool_slug)
      .maybeSingle();
    if (!account?.enabled || account.status !== "working" || !unexpired(account.expires_at))
          ...
          : await launchBrowserUseSessionOnly(admin, {
              ...
              viewport:
                data.viewport_width && data.viewport_height
                  ? { width: data.viewport_width, height: data.viewport_height }
                  : undefined,
            });
