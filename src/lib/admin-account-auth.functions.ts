...
export const adminRefreshAccountAuthentication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!loginUrl || new URL(loginUrl).protocol !== "https:")
    ...
    try {
      ...
      if (!cdp)
        throw new Error(
          "Login completed but the authenticated browser could not be reconnected for capture.",
        );
      try {
      ...
      await (admin as any)
        ...
        .update({
          status: "failed",
          error_code: e instanceof Error ? e.message.slice(0, 120) : "admin_refresh_failed",
          updated_at: new Date().toISOString(),
        })
        .eq("id", row.id);
      throw e;
