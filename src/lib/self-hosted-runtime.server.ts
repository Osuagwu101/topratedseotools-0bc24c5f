...
export async function launchSelfHostedBrowser(writerId: string, toolSlug: string): Promise<SelfHostedLaunch> {
  ...
  const body = JSON.stringify({ writer_id: writerId, tool_slug: toolSlug });
  const timestamp = Math.floor(Date.now() / 1000);
  const nonce = randomBytes(18).toString("base64url");
  const signature = signRuntimeRequest({ method: "POST", path, writerId, body, timestamp, nonce, secret });
  const response = await fetch(base + path, {
    method: "POST", headers: { "Content-Type": "application/json", "X-Toprated-Timestamp": String(timestamp), "X-Toprated-Nonce": nonce, "X-Toprated-Writer-Id": writerId, "X-Toprated-Signature": signature },
