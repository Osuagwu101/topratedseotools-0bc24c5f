...
{
  assert(
    classifyHealth(acc({ status: "login_failed" }), settings, now) === "login_problem",
    "login_failed => login_problem",
  );
  assert(
    classifyHealth(acc({ status: "password_changed" }), settings, now) === "login_problem",
    "password_changed => login_problem",
  );
}
// 7. unhealthy alert lists affected customer count
{
  const accounts = [
    acc({ id: "aa", status: "login_failed", active_count: 4, max_capacity: 10, fill_pct: 40 }),
  ];
...
// 8. disabled-with-customers alert
{
  ...
  const alerts = buildAccountAlerts(accounts, [], settings, now);
  assert(
    alerts.some((x) => x.kind === "disabled_with_customers"),
    "disabled + customers => alert",
  );
}
// 9. awaiting-assignment alert for paid orders
{
  ...
  assert(
    alerts.some((x) => x.kind === "awaiting_assignment" && x.order_id === "o1"),
    "awaiting_assignment alert emitted",
  );
}
// 10. needs_capacity_review alert
{
  ...
  const alerts = buildAccountAlerts(accounts, [], settings, now);
  assert(
    alerts.some((x) => x.kind === "needs_capacity_review"),
    "review alert emitted",
  );
}
// 11. per-tool summary
{
...
// 13. bulk plan never overfills
{
  ...
  for (const a of plan.assignments)
    if (a.target_account_id)
      perTarget[a.target_account_id] = (perTarget[a.target_account_id] ?? 0) + 1;
...
// 15. suspended alert distinct from expired
{
  const alerts = buildAccountAlerts(
    [acc({ id: "s1", status: "suspended", active_count: 1 })],
    [],
    settings,
    now,
  );
  assert(
    alerts.some((x) => x.kind === "suspended"),
    "suspended alert",
  );
}
// 16. deterministic alert keys (dedup contract)
{
  ...
  assert(
    first?.key === second?.key && first?.key === "account:dup:full",
    "alert keys stable for dedup",
  );
}
